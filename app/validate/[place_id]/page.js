    ...
    try {
      ...
      const [placeResult, imagesResult, reviewsResult, decisionResult] = await Promise.all([
        ...
        supabase
          ...
          .limit(10), // Only fetch top 10 reviews
        supabase
          .from('decisions')
          .select('decision')
          .eq('place_id', placeId)
      ...
      // Try to get images from place_images table first
      if (!imagesResult.error && imagesResult.data && imagesResult.data.length > 0) {
        setImages(imagesResult.data);
      } else if (placeData.raw?.imagesData) {
        // Fallback: extract from raw JSONB field
      ...
      <div className="relative min-h-screen bg-zinc-50 dark:bg-black">
        <div className="absolute inset-0 flex items-center justify-center bg-black/30 z-50">
          <div className="text-center">
            ...
            <p className="text-lg text-zinc-600 dark:text-zinc-400">Loading place...</p>
          </div>
        </div>
